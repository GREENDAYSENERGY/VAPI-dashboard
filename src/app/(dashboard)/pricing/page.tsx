...
import { calcCost, calcBlocks, formatDuration, RATE_PER_BLOCK, BLOCK_SECONDS } from "@/lib/pricing";
...
export default function PricingPage() {
  ...
  const totalRevenue = calls.reduce(
    (acc, c) => acc + calcCost(c.durationSeconds ?? 0),
    0
  );
  const totalVapiCost = calls.reduce((acc, c) => acc + (c.cost ?? 0), 0);
  const grossMargin = totalRevenue - totalVapiCost;
  ...
  const costPerConnected = connectedCalls.length
    ? totalVapiCost / connectedCalls.length
    : 0;
  ...
  for (const c of calls) {
    ...
    dispositionMap[d].duration += c.durationSeconds ?? 0;
    dispositionMap[d].revenue += calcCost(c.durationSeconds ?? 0);
  ...
  return (
    <div className="p-6 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Pricing Breakdown</h1>
        <p className="text-sm text-gray-500 mt-0.5">
          ${RATE_PER_BLOCK.toFixed(2)} per {BLOCK_SECONDS}-second block · rounded up
        </p>
      </div>
        ...
        <CardContent>
          ...
          {inputSeconds > 0 && (
            <p className="text-xs text-gray-400 mt-3">
              ...
              ceil = {calcBlocks_} blocks × ${RATE_PER_BLOCK.toFixed(2)} = ${calcCost_.toFixed(2)}
            </p>
          )}
        </CardContent>
      </Card>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs text-gray-500">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right">VAPI Cost</th>
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                <tr className="bg-gray-50 font-semibold border-t-2 border-gray-200">
                  ...
                  <td className="px-3 py-2 text-right font-mono">
                    {formatDuration(calls.reduce((a, c) => a + (c.durationSeconds ?? 0), 0))}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-green-700">
                    ${totalRevenue.toFixed(2)}
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-gray-500">
                    ${totalVapiCost.toFixed(2)}
                  </td>
